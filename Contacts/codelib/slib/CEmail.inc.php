...
class CEmail
{
	...
	function base64_encode_and_explode( $s )
	{
		return chunk_explode( base64_encode( $s ), 72 );
	}
	...
	function CreateEmailData()
	{
		...
		if ( $this->NoMultiPart() )
		{
			...
			$s = $this->base64_encode_and_explode( $s );
			$s .= "\r\n\r\n";
		...
		else
		{
			...
			if ( isset( $this->params['Attachment'] ) )
			{
				foreach ( $this->params['Attachment'] as $attachment )
				{
					...
					//--- Use chunk split
					$filedata = chunk_explode( base64_encode( $filedata ) );
